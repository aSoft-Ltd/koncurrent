...
@JsName("later_catch")
actual inline fun <T> Later<T>.catch(executor: Executor, noinline recover: (Throwable) -> T): Later<out T> = unsafeCast<MutablePromise<T>>().catch(recover)

@JsName("later_finally") // so that js does not call the underline finally
actual inline fun <T> Later<out T>.finally(executor: Executor, crossinline cleanUp: (state: Result<T>) -> Unit): Later<out T> {
