...
actual interface Later<out T> : Thenable<T> {
    ...
    fun <R> then(resolver: Function1I1R<T, R>, recover: Function1I1R<Throwable, R>, executor: Executor): Later<R>
    ...
    fun <R> then(resolver: Function1I1R<T, R>, recover: Function1I1R<Throwable, R>): Later<R>
    ...
    fun <R> then(onResolved: Function1I1R<T, R>, executor: Executor): Later<R>
    ...
    override fun <R> then(onResolved: Function1I1R<T, R>): Later<R>

    @JvmSynthetic
    actual override fun <R> andThen(onResolved: (T) -> Thenable<R>, executor: Executor?): Later<R>

    fun <R> andThen(resolver: Function1I1R<T, Thenable<R>>, executor: Executor): Later<R>

    @JvmSynthetic
    actual override fun <R> andThen(onResolved: (T) -> Thenable<R>): Later<R>

    override fun <R> andThen(onResolved: Function1I1R<T, Thenable<R>>): Later<R>

    actual override fun catch(handler: (Throwable) -> @UnsafeVariance T, executor: Executor): Later<T>

    fun error(handler: Function1I1R<Throwable, @UnsafeVariance T>, executor: Executor): Later<T>

    actual override fun catch(handler: (Throwable) -> @UnsafeVariance T): Later<T>

    override fun error(handler: Function1I1R<Throwable, @UnsafeVariance T>): Later<T>

    actual fun complete(cleaner: (state: Result<T>) -> Any?, executor: Executor?): Later<T>

    fun complete(completer: Callback<Result<T>>, executor: Executor): Later<T>
    ...
    companion object {
        @JvmStatic
        @JvmOverloads
        fun <T> of(value: T, executor: Executor = SynchronousExecutor): Later<T> = SuccessfulLater(value, executor)
    }
}