...
interface Thenable<out T> {
    ...
    fun <R> then(onResolved: Function<T, R>, executor: Executor): Thenable<R> = then(onResolved::invoke, null, executor)
    ...
    fun <R> then(onResolved: Function<T, R>): Thenable<R> = then(onResolved::invoke, null)
    ...
    fun <R> andThen(onResolved: Function<T, Thenable<R>>, executor: Executor): Thenable<R> = andThen(onResolved::invoke, executor)

    @JsName("_ignore_andThenFunction_T_Thenable")
    fun <R> andThen(onResolved: Function<T, Thenable<R>>): Thenable<R> = andThen(onResolved::invoke)
    ...
    fun error(handler: Function<Throwable, @UnsafeVariance T>, executor: Executor): Thenable<T> = then(null, handler::invoke, executor)

    @JsName("_ignore_errorFunction")
    fun error(handler: Function<Throwable, @UnsafeVariance T>): Thenable<T> = then(null, handler::invoke)
}